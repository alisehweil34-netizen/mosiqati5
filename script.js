...
(function() {
  ...
  // الأصوات تُحمَّل بشكل غير متزامن في بعض المتصفحات (مثل Chrome)،
  // لذلك نجهزها مبكراً ونستمع لحدث voiceschanged بدل الاعتماد على أول استدعاء فقط
  function loadVoices() {
  ...
  // يبحث عن أفضل صوت مطابق للغة المطلوبة (ar أو en) بدل الاعتماد فقط على utterance.lang
  function findVoice(targetLang) {
    var voices = cachedVoices.length ? cachedVoices : window.speechSynthesis.getVoices();
    if (!voices || !voices.length) return null;

    // تطابق كامل مثل ar-SA / ar-EG أولاً
    var exact = voices.find(function(v) { return v.lang && v.lang.toLowerCase() === targetLang.toLowerCase(); });
    if (exact) return exact;

    // ثم أي صوت يبدأ بنفس رمز اللغة (ar-*, en-*)
    var prefix = targetLang.split('-')[0].toLowerCase();
  ...
  function speak(text) {
    ...
    var targetLang = lang === 'ar' ? 'ar-SA' : 'en-US';
    ...
    var doSpeak = function() {
      ...
      if (lang === 'ar' && !voice) {
        // لا يوجد صوت عربي مثبت على هذا الجهاز/المتصفح
        var msg = '⚠️ لا يوجد صوت عربي مثبت على جهازك. ' +
                   'على ويندوز: أضِف حزمة اللغة العربية من إعدادات النطق (Speech). ' +
                   'على أندرويد: ثبّت "Google Text-to-Speech" وأضِف اللغة العربية من إعداداته.';
    ...
    // إن لم تُحمَّل الأصوات بعد، ننتظر قليلاً ثم نحاول مجدداً
    if (!cachedVoices.length && window.speechSynthesis.getVoices().length === 0) {
      setTimeout(function() {
        loadVoices();
        doSpeak();
      }, 150);
